...
$('.myForm').on('submit', function(e){
  ...
  if ( !firstName.val() || !lastName.val() || !late.val() || !tasks.val() || !present.val() || !interview.val() ||  !test.val()){
...
tbody.on('click', '#btn-edit', function(e){
  ...
  $.ajax({
    ...
    success: function(data){
      ...
      .then(data => {
        arr.push(+data.late, +data.tasks, +data.presentation, +data.interview, +data.test)
        console.log(arr)
...
$('.prev-btn-search').on('click', function(){
  ...
  --pageSearch;
  fetch(`http://localhost:8000/students?_page=${pageSearch}&_limit=5&q=${inpVal2}`)
    .then(result => result.json())
    .then(data => {
      if (data.length === 0) {
          alert('This is the last page! No next page!');
